...
Side-by-side excerpts for PipeWatch public docs. Formatting details: `pipewatch-docs/.cursor/rules/documentation.ai.mdc`.
...
> On **PipeWatch Cloud**, open **Settings → Billing** to compare plans and upgrade. Self-hosted instances do not include cloud billing screens — plan limits are configured by your operator.
...
> PipeWatch stores sessions in PostgreSQL via Drizzle ORM. The NestJS backend validates CSRF on mutating routes except the allowlist in `csrf.middleware.ts`.

**Problems:** Belongs in `pipewatch/docs/internal/` — readers are not PipeWatch developers (spec §2.4).

### Good

> PipeWatch keeps you signed in with a secure browser session. Sign out from **Settings → Account** to end your session on this device.
...
Welcome to PipeWatch!!! This page tells you everything about the product.
```
...
```mdx
---
title: "Quick start with self-hosted PipeWatch"
description: "Install PipeWatch, complete first-run setup, and save your first bookmark."
---
...
---
title: "Schnellstart"
description: "PipeWatch in wenigen Minuten einrichten."
---
```
...
---
title: "Quick start"
description: "Set up PipeWatch and save your first bookmark in minutes."
---
```

English only for all `pipewatch-docs/**/*.mdx`.
