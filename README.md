...
```bash
cp .env.example .env
bash scripts/generate-ce-secrets.sh --merge .env
```

The generator fills `POSTGRES_PASSWORD`, `REDIS_PASSWORD`, `JWT_SECRET`, `JWT_REFRESH_SECRET`, and `ENCRYPTION_KEY` when those keys are empty. Compose derives `DATABASE_URL` and `REDIS_URL` from the Postgres/Redis credentials — do not hardcode connection strings.

Add your GitHub App credentials to `.env` manually. See [PRD §23](docs/internal/PipeWatch_MVP_PRD.md) for the full variable reference.

| Variable | Required | Notes |
|---|---|---|
| `POSTGRES_PASSWORD`, `REDIS_PASSWORD` | Yes | Generated by `scripts/generate-ce-secrets.sh` (or `openssl rand -hex 32`) |
| `GITHUB_APP_*`, `GITHUB_CLIENT_*`, `GITHUB_WEBHOOK_SECRET` | Yes | From your GitHub App |
| `JWT_SECRET`, `JWT_REFRESH_SECRET`, `ENCRYPTION_KEY` | Yes | Generated by the script above (≥32 chars) |
