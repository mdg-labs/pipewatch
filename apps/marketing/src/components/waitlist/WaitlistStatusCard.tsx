...
type WaitlistStatusCardProps = {
  ...
  umamiAction?: "confirm" | "unsubscribe";
};

export function WaitlistStatusCard({ variant, umamiAction }: WaitlistStatusCardProps) {
  const content = getStatusContent(variant);
  const isError = variant === "invalid_expired" || variant === "network_error";

  useEffect(() => {
    if (umamiAction) {
      trackUmamiEvent(`waitlist-${umamiAction}`, { status: variant });
    }
  }, [umamiAction, variant]);
